type role = Guest | Moderator | Admin [@@deriving show]
[@@deriving_inline queryable]
let _ = fun (_ : role) -> ()
let converter_role =
  let decode =
    function
    ...
    | "Admin" -> Ok Admin
    | _ as t -> Error (Format.sprintf "unknown case %s" t) in
  let encode =
    function | Guest -> "Guest" | Moderator -> "Moderator" | Admin -> "Admin" in
  Caqti_type.enum ~encode ~decode "role"
let _ = converter_role
[@@@end]

type status = [ `Employee | `Director] [@@deriving show]
[@@deriving_inline queryable]
let _ = fun (_ : status) -> ()
let converter_status =
  let decode =
    ...
    | _ as t -> Error (Format.sprintf "unknown case %s" t) in
  let encode = function | `Employee -> "Employee" | `Director -> "Director" in
